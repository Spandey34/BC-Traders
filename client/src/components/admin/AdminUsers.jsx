...
const AdminUsers = () => {
  ...
  const verifyUser = async (userToVerify) => {
    ...
    setUsers((prev) =>
      prev.map((u) =>
        u._id === userToVerify._id ? { ...u, isVerified: true } : u
      )
    );
    try {
      await axios.post(
  ...
  return (
    <div className="p-4 md:p-6 pb-24 lg:pb-6">
      ...
      <ConfirmationDialog
        ...
        onConfirm={() => confirmAction("delete")}
        title="Confirm Deletion"
        confirmText="Yes, Delete User"
        confirmColor="bg-red-600 hover:bg-red-700"
      >
        Are you sure you want to permanently delete {deleteConfirm.user?.name}?
        This action cannot be undone.
      </ConfirmationDialog>
      <ConfirmationDialog
        isOpen={verifyConfirm.isOpen}
        onClose={() => setVerifyConfirm({ isOpen: false })}
        onConfirm={() => confirmAction("verify")}
            ...
            >
              ...
              <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
                <div className="flex items-center gap-4">
                  {/* MODIFICATION START: Added relative container and online indicator */}
                  <div className="relative">
                    ...
                    {onlineUsers?.includes(user._id) && (
                      <span className="absolute bottom-0 right-0 block h-3 w-3 rounded-full bg-green-500 border-2 border-white dark:border-gray-800" />
                    )}
                  </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center border-t border-b dark:border-gray-700 py-3 my-3 text-sm">
                <div>
                  ...
                  <span className="font-semibold">
                    ₹{user.totalPaidAmount?.toFixed(2) || "0.00"}
                  </span>
                </div>
                <div>
                  <span className="block text-gray-500 text-xs">
                  ...
                  <span className="font-semibold text-yellow-500">
                    ₹{user.unpaidAmount?.toFixed(2) || "0.00"}
                  </span>
                </div>
                <div>
                  <span className="block text-gray-500 text-xs">
                ...
                <div>
                  <span className="block text-gray-500 text-xs">Last Amt</span>
                  {user.lastOrderAmount > 0
                    ? `₹${user.lastOrderAmount.toFixed(2)}`
                    : "N/A"}
                </div>
              </div>
              ...
              <div className="pt-2 flex flex-wrap gap-2 justify-end items-center min-h-[42px]">
                {user.isVerified ? (
                  <span className="px-4 py-2 text-xs font-bold text-green-800 bg-green-100 rounded-full">
                    Verified
                  </span>
                ) : (
                  <button
                    onClick={() => setVerifyConfirm({ isOpen: true, user })}
                    className="px-4 py-2 text-sm font-semibold text-white rounded-md shadow-sm bg-blue-500 hover:bg-blue-600"
                  >
                    Verify
                  </button>
                )}
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold text-white rounded-md shadow-sm bg-red-500 hover:bg-red-600"
                >
                  Delete
                </button>
              </div>
